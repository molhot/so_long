...
/*   Updated: 2023/01/03 20:13:40 by satushi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool map_surround_check_helper2(char *line, size_t rowmax)
{
	...
	while (line[position] != '\0' && line[position] != '\n')
	{
		if ((position == 0 || position == rowmax) && mapinfo[position] != '1')
			return (false);
		position++;
	}
	return (true);
...
bool    map_basiccheck(char **mapinfo)
{
    ...
    while (mapinfo[map_position] != NULL)
        map_position++;
    if (map_rowcheck(mapinfo) != true) //mapの列の確認
        return (false);
    ...
    if (map_surroundcheck(mapinfo, map_high, map_row) != true)
		return (false);
	if (fill_check(mapinfom map_high) != true)
		return (false);
    return (true);
}